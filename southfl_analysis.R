...
loss_summary = readRDS("Summaries and Extractions/loss_summary.rds")
bank_summary = readRDS("Summaries and Extractions/bank_summary.rds")
...
# Create a named list to store the reprojected rasters
lossmosaics <- list()

# Loop through service area names and load + reproject each raster
for (sa in sa_names) {
  # Construct file path
  file_path <- file.path("CONUS/Wetland Loss/Service Area Mosaics Loss - 3.5", paste0(sa, "_lossmosaic.tif"))
  
  # Check if file exists to avoid errors
  if (file.exists(file_path)) {
    # Load and reproject
    loss_raster <- rast(file_path)
    loss_mosaics_4326[[sa]] <- terra::project(loss_raster, crs("EPSG:4326"))
  } else {
...
# Loop through each raster and extract as dataframe
for (name in names(lossmosaics)) {
  cat("Processing:", name, "\n")
  
  # Extract raster
  mosaic_rast <- lossmosaics[[name]]
  
  # Convert to data frame (with coordinates and drop NAs)
  mosaic_df <- as.data.frame(mosaic_rast, xy = TRUE, na.rm = TRUE)
  colnames(mosaic_df)[3:5] <- c("housing_units", "housing_value", "population")
  
  # Reproject coordinates to EPSG:4326
  coords_4326 <- terra::project(cbind(mosaic_df$x, mosaic_df$y), crs(mosaic_rast), "EPSG:4326")
  
  # Create reprojected dataframe
  mosaic_df_4326 <- data.frame(
    x = coords_4326[, 1],
    y = coords_4326[, 2],
    housing_units = mosaic_df$housing_units,
    housing_value = mosaic_df$housing_value,
    population = mosaic_df$population
  )
  
  # Store in list
  mosaic_df_list[[name]] <- mosaic_df_4326
}


states <- ne_states(country = "United States of America", returnclass = "sf")
states_albers <- st_transform(states, crs(top10_sas))
states_vect <- vect(states_albers)
intersected <- terra::intersect(states_vect, top10_sas)
intersected$name

fl_sf <- subset(states_albers, name %in% "Florida")

top10_sa <- st_as_sf(top10_sa)
top10_banks = st_as_sf(top10_sa)

# Note: ggspatial assumes your map is in lon/lat (EPSG:4326), so reproject everything
target_crs <- 4326

# Reproject everything to WGS84 (EPSG:4326)
top10_sa_4326 <- st_transform(top10_sa, target_crs)
top10_bank_4326 <- st_transform(top10_banks, target_crs)
...
# Define directories
sa_dir <- "Extraction and Summaries/Extract Service Areas"
bank_dir <- "Extraction and Summaries/Extract Banks"
loss_dir <- "Wetland Loss/Extraction and Summaries/Extract Service Area Loss"
...
# ------------------------------------------------------------------
# 1. Tweak labels & factor order
# ------------------------------------------------------------------
plot_box <- plot_df %>% 
  mutate(
    group = factor(group,
                   levels = c("loss", "bank", "service_area"),
                   labels = c("Wetland Loss", "Bank", "Remaining Wetland")),
    bank_name = factor(bank_name, levels = names(bank_name_labels))
  )

# ------------------------------------------------------------------
# 2. Box-plots for downstream housing units (log-scale)
# ------------------------------------------------------------------
ggplot(plot_box,
       aes(x = group, y = log_hu, fill = group)) +
  geom_boxplot(width = .55, outlier.alpha = .25) +
  facet_wrap(~ bank_name,
             scales   = "free_y",          # each bank can use its own y-range
             ncol     = 3,
             labeller = labeller(bank_name = bank_name_labels)) +
  labs(
    x = NULL,
    y = "Log 10 (downstream housing units)"
  ) +
  scale_fill_manual(
    values = c("Wetland Loss"      = "red",
               "Remaining Wetland" = "blue",
               "Bank"              = "#00BA38"),
    guide  = "none"                # legend not really needed here
  ) +
  theme_bw(base_size = 14) +
  theme(
    panel.grid.major = element_blank(),
